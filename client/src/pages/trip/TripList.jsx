...
const TripList = ({ search }) => {
    ...
    const fetchTrips = async () => {
        ...
        try {
            const response = await fetch(`https://52.78.2.148:80/api/schedules?page=1&sortBy=${sortBy}&search=${encodeURIComponent(search)}`, {
                method: "GET",
                headers: {
    ...
    const submitNewImage = async () => {
        ...
        try {
            ...
            const response = await fetch(`https://52.78.2.148:80/api/schedules/${editingImage}/files`, { // URL 수정
                method: "PATCH",
                headers: {
                    "Authorization": `Bearer ${accessToken}`
                },
